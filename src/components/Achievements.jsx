...
const achievements = [
    ...
    {
        id: 3,
        title: 'Best Performer - Samadhan 2.0',
    ...
    {
        id: 4,
        title: 'Adobe India Hackathon - Final Round',
    ...
    {
        id: 5,
        title: 'Nirman Hackathon - Final Round',
    ...
    },
    {
        ...
        description: 'Successfully cleared the Smart India Hackathon at college level.',
        icon: '🏆',
        rank: 'bronze',
        images: [sihCollege],
    },
...
const rankStyles = {
    gold: {
        shadow: '0 10px 30px -8px rgba(234, 179, 8, 0.2)',
        border: 'border-yellow-200',
        badge: 'bg-gradient-to-r from-yellow-400 to-amber-500',
        accent: 'text-yellow-600',
    },
    silver: {
        shadow: '0 10px 30px -8px rgba(148, 163, 184, 0.25)',
        border: 'border-slate-200',
        badge: 'bg-gradient-to-r from-slate-300 to-slate-400',
        accent: 'text-slate-500',
    },
    bronze: {
        shadow: '0 10px 30px -8px rgba(180, 83, 9, 0.15)',
...
const ImageGallery = ({ images, title, onOpenLightbox }) => {
    ...
    return (
        <div className="relative group overflow-hidden rounded-t-2xl">
            ...
            <div className="absolute bottom-2 right-2 px-2 py-1 bg-white/80 backdrop-blur-sm rounded text-xs text-slate-600 font-medium">
...
const Achievements = () => {
    ...
    const openLightbox = (images, index) => setLightboxData({ open: true, images, index });
    const closeLightbox = () => setLightboxData({ open: false, images: [], index: 0 });
    const nextImage = (e) => { e?.stopPropagation(); setLightboxData(prev => ({ ...prev, index: (prev.index + 1) % prev.images.length })); };
    const prevImage = (e) => { e?.stopPropagation(); setLightboxData(prev => ({ ...prev, index: (prev.index - 1 + prev.images.length) % prev.images.length })); };

    return (
        <section id="achievements" className="section relative overflow-hidden" style={{ background: '#f8fafc' }}>
            ...
            <div className="container-custom relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {achievements.map((achievement, index) => {
                        const style = rankStyles[achievement.rank] || rankStyles.bronze;
                        return (
                            <motion.div
                                key={achievement.id}
                                initial={{ opacity: 0, y: 30 }}
                                ...
                                whileHover={{ y: -4 }}
                                className={`card overflow-hidden ${style.border}`}
                            ...
                            >
                                ...
                                <div className="p-6">
                                    <div className="flex items-start gap-3 mb-3">
                                        ...
                                        <div>
                                            <h3 className="text-lg font-bold text-heading font-grotesk leading-tight">
                                                {achievement.title}
                                            </h3>
                                        </div>
                                    </div>
                ...
                <AnimatePresence>
                    {lightboxData.open && (
                        ...
                        >
                            <button onClick={closeLightbox} className="absolute top-4 right-4 p-2 rounded-full bg-white/20 text-white hover:bg-white/30 transition-colors z-10">
                                <X size={24} />
                            </button>
                            ...
                            >
                                <img src={lightboxData.images[lightboxData.index]} alt={`Image ${lightboxData.index + 1}`} className="w-full h-full object-contain rounded-xl" />
                                ...
                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                                    {lightboxData.images.map((_, index) => (
                                        <button key={index} onClick={(e) => { e.stopPropagation(); setLightboxData(prev => ({ ...prev, index })); }}
                                            className={`w-3 h-3 rounded-full transition-all ${index === lightboxData.index ? 'bg-white scale-125' : 'bg-white/50'}`} />
                                    ))}
                                </div>

                                <div className="absolute bottom-4 right-4 px-3 py-1.5 bg-white/20 backdrop-blur-sm rounded-lg text-sm text-white">
